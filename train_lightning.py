...
def main(args):
    ...
    train_loader, val_loader, test_loader, train_size, val_size, test_size = (
        setup_data_loaders(config, tokenizer)
    )

    logger.info(
        f"Data loaded - Train: {train_size}, Val: {val_size}, Test: {test_size}"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train diacritization model with PyTorch Lightning"
    )
    parser.add_argument(

...
def evaluate(model, dataloader, criterion, device, epoch=0):
    ...
    with torch.no_grad():
        ...
        data_iter.set_postfix(
            ...
            loss=validation_loss / len(dataloader),
        )
    ...
    for epoch in range(config.TRAIN.NUM_EPOCHS):
        ...
        if (epoch + 1) % config.TRAIN.EVAL_FREQ == 0:
            val_loss, val_accuracy = evaluate(
                model, val_loader, criterion, config.TRAIN.DEVICE, epoch=epoch + 1
            )
...
def main(configs):
    ...
    expanded_vocab = expand_vocabulary(
        constants.characters_mapping, constants.classes_mapping
    )

    assert configs.MODEL.VOCAB_SIZE == len(
        constants.characters_mapping
    ), f"model vocab size {configs.MODEL.VOCAB_SIZE} does not match characters mapping size {len(constants.characters_mapping)}"
    assert configs.MODEL.ASR_VOCAB_SIZE == len(
        expanded_vocab
    ), f"model asr vocab size {configs.MODEL.ASR_VOCAB_SIZE} does not match expanded vocab size {len(expanded_vocab)}"
    assert configs.MODEL.OUTPUT_SIZE == len(
        constants.classes_mapping
    ), f"model output size {configs.MODEL.OUTPUT_SIZE} does not match classes mapping size {len(constants.classes_mapping)}"
    ...
    train_data = TextAudioDataset(
        configs.DATA.TRAIN_PATH, expanded_vocab, max_length=configs.DATA.MAX_LENGTH
    )
    test_data = TextAudioDataset(
        configs.DATA.TEST_PATH, expanded_vocab, max_length=configs.DATA.MAX_LENGTH
    )
    ...
    if not configs.DATA.VAL_PATH:
        ...
        val_data = TextAudioDataset(
            configs.DATA.VAL_PATH, expanded_vocab, max_length=configs.DATA.MAX_LENGTH
        )
    ...
    logger.info(
        f"Training samples: {len(train_data)}, Validation samples: {len(val_data)}, Test samples: {len(test_data)}"
    )
    ...
    if configs.MODEL.PRETRAINED_PATH:
        model.load_pretrained(
            configs.MODEL.PRETRAINED_PATH,
            text_branch_only=configs.MODEL.LOAD_TEXT_BRANCH_ONLY,
        )
    ...
    best_val_loss, _ = train(
        configs, model, train_loader, val_loader, criterion, optimizer, writer
    )

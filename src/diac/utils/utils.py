...
def load_constants(aux_dataset_path: str, with_extra_train: bool = False) -> Constants:
    ...
    global constants
    if with_extra_train:
        constants.characters_mapping = pkl.load(
            open(
                os.path.join(aux_dataset_path, "RNN_BIG_CHARACTERS_MAPPING.pickle"),
                "rb",
            )
        )
    else:
        constants.characters_mapping = pkl.load(
            open(
                os.path.join(aux_dataset_path, "RNN_SMALL_CHARACTERS_MAPPING.pickle"),
                "rb",
            )
        )
    constants.arabic_letters_list = pkl.load(
        open(os.path.join(aux_dataset_path, "ARABIC_LETTERS_LIST.pickle"), "rb")
    )
    constants.diacritics_list = pkl.load(
        open(os.path.join(aux_dataset_path, "DIACRITICS_LIST.pickle"), "rb")
    )
    constants.classes_mapping = pkl.load(
        open(os.path.join(aux_dataset_path, "RNN_CLASSES_MAPPING.pickle"), "rb")
    )
    constants.rev_classes_mapping = pkl.load(
        open(os.path.join(aux_dataset_path, "RNN_REV_CLASSES_MAPPING.pickle"), "rb")
    )
    return constants
    ...
    for line in data_raw:
        ...
        for idx, char in enumerate(line):
            if char in constants.diacritics_list:
                continue
            x.append(
                constants.characters_mapping.get(
                    char, constants.characters_mapping["<PAD>"]
                )
            )
            if char not in constants.arabic_letters_list:
                ...
                if idx + 1 < len(line) and line[idx + 1] in constants.diacritics_list:
                    ...
                    if (
                        idx + 2 < len(line)
                        and line[idx + 2] in constants.diacritics_list
                    ):
                ...
                y.append(
                    constants.classes_mapping.get(
                        char_diac, constants.classes_mapping[""]
                    )
                )
...
def expand_vocabulary(
    CHARACTERS_MAPPING: Dict[str, int], CLASSES_MAPPING: Dict[str, int]
) -> Dict[str, int]:
    """
    ...
    for diacritic in CLASSES_MAPPING:
        if diacritic not in ExpandedVocabulary and diacritic.strip() not in [
            "",
            "<N/A>",
        ]:
...
def map_asr_data(
    data_raw: List[str], expanded_vocabulary: Dict[str, int]
) -> List[List[int]]:
    """
...
def batch_decode_predictions(
    batch_predictions: List[List[int]], texts: List[str]
) -> List[str]:
    return [
        decode_predictions(pred, text) for pred, text in zip(batch_predictions, texts)
    ]
...
def decode_predictions(predictions: List[int], text: str) -> str:
    ...
    for idx, char in zip(predictions, text):
        ...
        if idx in [
            constants.classes_mapping.get("<SOS>"),
            constants.classes_mapping.get("<EOS>"),
            constants.classes_mapping.get("<PAD>"),
        ]:
            continue
...
def setup_data_loaders(config, tokenizer):
    ...
    train_data = TextAudioDataset(
        config.DATA.TRAIN_PATH, tokenizer, max_length=config.DATA.MAX_LENGTH
    )

    test_data = TextAudioDataset(
        config.DATA.TEST_PATH, tokenizer, max_length=config.DATA.MAX_LENGTH
    )

    # Handle validation data
    if hasattr(config.DATA, "VAL_PATH") and config.DATA.VAL_PATH:
        val_data = TextAudioDataset(
            config.DATA.VAL_PATH, tokenizer, max_length=config.DATA.MAX_LENGTH
        )
    else:
...
def setup_loggers(config):
    ...
    for name in lightning_logger_names:
        ...
        for h in root_handlers:
            if all(
                type(h) != type(existing)
                or getattr(h, "stream", None) != getattr(existing, "stream", None)

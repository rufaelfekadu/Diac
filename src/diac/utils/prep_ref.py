...
def load_file(file_path):
    ...
    try:
        ...
        if len(rows) > 0 and len(rows[0]) == 2:
            ...
            raise ValueError("File format not recognized. Expected TSV with 2 columns or single-column text file.")
...
def main():
    parser = argparse.ArgumentParser(description="Prepare reference data from TSV or text file")
    parser.add_argument("--input_file", required=True, help="Path to TSV file with reference and ASR transcriptions, or single-column text file")
    parser.add_argument("--output-dir", "-o", default="output", help="Output directory for processed files")

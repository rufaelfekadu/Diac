...
def calculate_wer(
    ...
    equal = 0
    not_equal = 0
    for idx, (original_line, target_line) in enumerate(
        zip(original_content, target_content)
    ):
        if style == "Fadel":
            original_line = clear_line(original_line, arabic_letters, diacritic_classes)
            target_line = clear_line(target_line, arabic_letters, diacritic_classes)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ofp", "--original-file-path", help="File path to original text", required=True
    )
    parser.add_argument(
        "-tfp", "--target-file-path", help="File path to target text", required=True
    )
    parser.add_argument(
        "-s",
        "--style",
    ...
    parser.add_argument(
        "--log_file", type=str, default="eval.log", help="Path to the log file"
    )
    ...
    logger.info("\n\n")
    logger.info(
        f"Evaluating files: {args.original_file_path} and {args.target_file_path} with style {args.style}"
    )
    logger.info(
        "+---------------------------------------------------------------------------------------------+"
    )

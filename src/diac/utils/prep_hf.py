...
def _get_Audio(audio_item):
    ...
    """
    if isinstance(audio_item, AudioDecoder) or (
        isinstance(audio_item, dict) and "array" in audio_item
    ):
...
def main(args):
    ...
    for ds in args.datasets:
        if ds not in AVAILABLE_DATASETS:
            print(
                f"Dataset '{ds}' not recognized. Available datasets: {', '.join(AVAILABLE_DATASETS.keys())}"
            )
            continue
        ...
        for split_name, splits in all_splits.items():
            ...
            for split in splits:
                try:
                    ...
                    print(
                        f"Failed to load dataset '{dataset_name}' with split '{split}' trying locally."
                    )
                    try:
                        ...
                        print(
                            f"Also failed to load from disk '{dataset_name}' with split '{split}'. Skipping..."
                        )
                        continue
                breakpoint()
                try:
                    temp_text_inputs, temp_results = transcribe_from_dataset(
                        dataset,
                    ...
                    print(
                        f"Error during transcription for dataset '{ds}' split '{split}': {e}"
                    )
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["clartts"],
        help="List of dataset names to process. Available: "

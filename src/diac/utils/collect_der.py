...
def parse_log(log_path: Path) -> Optional[DerRecord]:
    ...
    if (
        model is None or model_type is None or training_dataset is None
    ) and "results" in log_path.parts:
    ...
    model_full = (
        f"{model}-{model_type}"
        if model_type and not model.endswith(model_type)
        else model
    )
...
def collect(records: Iterable[DerRecord]) -> List[DerRecord]:
    ...
    return sorted(
        records,
        key=lambda r: (r.model_full, r.training_dataset, r.eval_set, r.log_path),
    )
...
COLUMNS = [
    ...
    "eval_set",
    "log_path",
    "der_w_case_incl_no_diac",
    "der_wo_case_incl_no_diac",
    "der_w_case_excl_no_diac",
    "der_wo_case_excl_no_diac",
]
...
def compute_extremes(records: List[DerRecord]) -> ExtremesMap:
    ...
    extremes: ExtremesMap = {
        col: {"min": set(), "max": set(), "second_max": set()} for col in METRIC_COLUMNS
    }
...
def output_csv(
    ...
    extremes: Optional[ExtremesMap] = None,
...
def output_markdown(
    ...
    for title, subs in metric_groups:
        lines.append(
            f'      <th colspan="{len(subs)}" style="text-align:center">{esc(title)}</th>'
        )
    lines.append("    </tr>")
        ...
        for j, v in enumerate(row[len(base_cols) :]):
            ...
            if extremes is not None and (mark_lowest or mark_highest or mark_second):
                ...
                if mark_second and row_idx in extremes[col_name]["second_max"]:
                    styles.append(
                        "background:#fef3c7;text-decoration:underline"
                    )  # amber underline
                    title_tags.append("2nd")
                if mark_highest and row_idx in extremes[col_name]["max"]:
                    styles.append(
                        "background:#fee2e2;font-style:italic"
                    )  # reddish italic
...
def output_latex(
    ...
    for _group_title, cols in metrics:
        ...
        for c in cols:
            if (
                c.endswith("_w_case_incl_no_diac")
                or c.endswith("_w_case_excl_no_diac")
                or "w_case" in c
            ):
                subs.append("With case")
            elif (
                c.endswith("_wo_case_incl_no_diac")
                or c.endswith("_wo_case_excl_no_diac")
                or "wo_case" in c
            ):
            ...
            if not v:
                ...
                try:
                    ...
                    if extremes is not None and (
                        mark_lowest or mark_highest or mark_second
                    ):
                        col_name = METRIC_COLUMNS[j]
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect DER metrics from evaluation logs."
    )
    parser.add_argument(
        "--results_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional output file path (CSV or Markdown depending on --format)",
    )
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "--mark-lowest",
        action="store_true",
        help="Annotate lowest values per metric column",
    )
    parser.add_argument(
        "--mark-highest",
        action="store_true",
        help="Annotate highest values per metric column",
    )
    parser.add_argument(
        "--mark-second-highest",
        action="store_true",
        help="Annotate second-highest values per metric column",
    )
    ...
    extremes = (
        compute_extremes(collected)
        if (args.mark_lowest or args.mark_highest or args.mark_second_highest)
        else None
    )

...
class TokenAndPositionEmbedding(nn.Module):
    ...
    def forward(self, inputs):
        ...
        div_term = torch.exp(
            torch.arange(0, e, 2, device=inputs.device) * -(math.log(10000.0) / e)
        )
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, num_heads, dff, rate=0.5):
        ...
        self.multi_head_attention = nn.MultiheadAttention(
            d_model, num_heads, dropout=rate, batch_first=True
        )
    ...
    def forward(self, inputs, mask=None):
        ...
        attention_output, _ = self.multi_head_attention(
            inputs, inputs, inputs, attn_mask=mask
        )
...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ASR branch
        if use_asr:
            self.asr_embedding = TokenAndPositionEmbedding(
                maxlen, asr_vocab_size, d_model
            )
    ...
    def forward(self, inputs, inputs_asr=None, **kwargs):
        ...
        assert (
            inputs_asr is not None
        ), "ASR inputs must be provided when use_asr is True"
        # ASR branch
        ...
        cross_out, _ = self.cross_attention(
            x.transpose(0, 1), asr_emb.transpose(0, 1), asr_emb.transpose(0, 1)
        )
    ...
    def load_pretrained(self, pretrained_model_path, text_branch_only=False):
        ...
        if not pretrained_model_path:
            print(
                "No pretrained model path provided, skipping loading pretrained weights."
            )
            return self
        ...
        try:
            ...
            if text_branch_only:
                pretrained_dict = {
                    k: v for k, v in pretrained_dict.items() if k.startswith("text_")
                }
            ...
            print(
                f"Loaded pretrained weights from {pretrained_model_path} with text_branch_only={text_branch_only}"
            )

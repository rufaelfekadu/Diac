...
class DiacritizationModule(L.LightningModule):
    ...
    def from_pretrained(cls, repo_or_dir, config=None, tokenizer=None, tokenizer_constants_path=None,
                        map_location='cpu', device=None, hf_token=None):
        ...
        # find checkpoint
        candidates = ['pytorch_model.bin', 'model.pt', 'model.pth', 'state_dict.pt', 'final_model.pt', 'best_model.ckpt']
        ckpt_path = None
        for c in candidates:
            p = _download_file(c)
            if p is not None:
        ...
        if config is None:
            ...
            if cfg_path:
                try:
                    ...
                    print('warning: failed to load config from file, using default config')
        ...
        if tokenizer is None and tokenizer_constants_path is None:
            try:
                ...
                raise RuntimeError("Failed to load tokenizer from constants.json in the repo.")
        ...
        try:
            ...
            module = cls.load_from_checkpoint(ckpt_path, config=config, tokenizer=tokenizer, map_location=map_location)
        except Exception as e:
            raise RuntimeError(f"Failed to load checkpoint with Lightning's load_from_checkpoint: {e}")
    ...
    def on_after_backward(self):
        ...
        if self.grad_clip_norm:
            try:
                clipped_norm = torch.nn.utils.clip_grad_norm_(params, self.grad_clip_norm)
            except Exception:
            ...
            if clipped_norm is not None:
                try:
                    self.log('grad_norm_clipped', clipped_norm, on_step=True, on_epoch=False, prog_bar=False)
                except Exception:

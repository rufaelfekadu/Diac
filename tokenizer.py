...
class ArabicDiacritizationTokenizer:
    ...
    def _load_constants_pkl(self) -> TokenizerConstants:
        ...
        if not os.path.isdir(self.constants_path):
            raise ValueError(
                f"Constants path {self.constants_path} is not a directory."
            )
        ...
        characters_mapping = pkl.load(
            open(os.path.join(self.constants_path, chars_file), "rb")
        )

        # Load other constants
        arabic_letters_list = pkl.load(
            open(os.path.join(self.constants_path, "ARABIC_LETTERS_LIST.pickle"), "rb")
        )
        diacritics_list = pkl.load(
            open(os.path.join(self.constants_path, "DIACRITICS_LIST.pickle"), "rb")
        )
        classes_mapping = pkl.load(
            open(os.path.join(self.constants_path, "RNN_CLASSES_MAPPING.pickle"), "rb")
        )
        rev_classes_mapping = pkl.load(
            open(
                os.path.join(self.constants_path, "RNN_REV_CLASSES_MAPPING.pickle"),
                "rb",
            )
        )

        # Create expanded vocabulary for ASR
        expanded_vocabulary = self._expand_vocabulary(
            characters_mapping, classes_mapping
        )
    ...
    def _expand_vocabulary(
        self, characters_mapping: Dict[str, int], classes_mapping: Dict[str, int]
    ) -> Dict[str, int]:
        ...
        for diacritic in classes_mapping:
            if diacritic not in expanded_vocab and diacritic.strip() not in [
                "",
                "<N/A>",
            ]:
    ...
    def encode_labels(self, text: str) -> List[int]:
        ...
        while idx < len(text):
            ...
            if char not in self.constants.arabic_letters_list:
                ...
                if (
                    idx + 1 < len(text)
                    and text[idx + 1] in self.constants.diacritics_list
                ):
                    ...
                    if (
                        idx + 2 < len(text)
                        and text[idx + 2] in self.constants.diacritics_list
                    ):
                        # Try both orders
    ...
    def encode(self, text: str, asr_text: str = None, return_tensor=False):
        ...
        if return_tensor:
            encoded_text = torch.tensor(encoded_text, dtype=torch.long).unsqueeze(0)
            encoded_asr = (
                torch.tensor(encoded_asr, dtype=torch.long).unsqueeze(0)
                if asr_text
                else None
            )
    ...
    def encode_batch(
        ...
        asr_tensors = [
            torch.tensor(encoded, dtype=torch.long) for encoded in encoded_asr_texts
        ]
    ...
    def decode(self, predictions: List[int], original_text: str) -> str:
        ...
        start_idx = (
            ...
            if (
                len(predictions) > 0
                and predictions[0] == self.constants.classes_mapping.get("<SOS>")
            )
            else 0
        )
    ...
    def decode_batch(
        self, predictions_batch: torch.Tensor, original_texts: List[str]
    ) -> List[str]:
        """
...
if __name__ == "__main__":
    ...
    encoded, encoded_asr, labels = tokenizer.encode_batch(
        [sample_txt], [sample_asr_text], padding=False
    )
